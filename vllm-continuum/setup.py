...
class cmake_build_ext(build_ext):
    ...
    def configure(self, ext: CMakeExtension) -> None:
        ...
        if _is_cuda():
            ...
            # conda-forge CUDA toolkit places headers/libs under
            # $CONDA_PREFIX/targets/<arch>-linux. Detect and pass it
            # explicitly so CMake/Torch can resolve CUDA_INCLUDE_DIRS.
            cuda_root = (
                os.environ.get("CUDAToolkit_ROOT")
                or os.environ.get("CUDA_TOOLKIT_ROOT_DIR")
            )
            if not cuda_root:
                ...
                if conda_prefix:
                    ...
                    if target:
                        candidate = Path(conda_prefix) / "targets" / target
                        if (candidate / "include" / "cuda.h").is_file():
                            cuda_root = str(candidate)
            if cuda_root:
                include_dir = Path(cuda_root) / "include"
                libcudart = Path(cuda_root) / "lib" / "libcudart.so"
                cmake_args += [f"-DCUDAToolkit_ROOT={cuda_root}"]
                cmake_args += [f"-DCUDA_TOOLKIT_ROOT_DIR={cuda_root}"]
                if include_dir.is_dir():
                    cmake_args += [f"-DCUDA_INCLUDE_DIRS={include_dir}"]
                if libcudart.is_file():
                    cmake_args += [f"-DCUDA_CUDART_LIBRARY={libcudart}"]
                logger.info("Using CUDA toolkit root: %s", cuda_root)
    ...
    def run(self):
        # Ensure destination package directory exists before editable wheel
        # copies compiled flash-attn extensions into source tree.
...
def get_vllm_version() -> str:
    # This project may be checked out as a subdirectory inside a parent git
    # repository. Point setuptools-scm at the parent so version discovery
    # works in editable installs.
    version = get_version(
        root=".",
