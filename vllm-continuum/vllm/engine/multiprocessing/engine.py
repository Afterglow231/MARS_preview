...
class MQLLMEngine:
    ...
    def _handle_utility_request(self, request: RPCUtilityRequest):
        try:
            target = getattr(self, request.method)
        ...
        if not callable(target):
            rpc_err = RPCError(
                request_id=request.request_id,
                is_engine_errored=False,
                exception=TypeError(f"Utility target {request.method!r} is not callable."),
            )
            self._send_outputs(rpc_err)
            return

        try:
            result = target(*tuple(request.args or ()))
        except BaseException as e:
            rpc_err = RPCError(request_id=request.request_id,
                               is_engine_errored=False,
                               exception=e)
        ...
        self._send_outputs(
            RPCUtilityResponse(request_id=request.request_id, result=result))
    ...
    def get_kv_cache_stats(self) -> dict[str, object]:
        try:
            block_size = int(getattr(self.engine.cache_config, "block_size", 0)
                             or 0)
            num_gpu_blocks = int(
                getattr(self.engine.cache_config, "num_gpu_blocks", 0) or 0)
            num_free_blocks = int(
                sum(scheduler.block_manager.get_num_free_gpu_blocks()
                    for scheduler in self.engine.scheduler))
            usage = None
            if num_gpu_blocks > 0:
                usage = max(
                    0.0,
                    min(1.0, 1.0 - (float(num_free_blocks) / float(num_gpu_blocks))),
                )
            return {
                ...
                "usage": usage,
                "total_tokens": max(0, num_gpu_blocks) * max(0, block_size),
                "free_tokens": max(0, num_free_blocks) * max(0, block_size),
            }
        except Exception as e:
