...
class EngineCore:
    ...
    def get_kv_cache_stats(self) -> dict[str, Any]:
        """Return KV cache capacity/usage stats.

        This is exposed via EngineCoreClient.call_utility_async and is intended
        for lightweight admission control in external drivers.
        """
        try:
            scheduler = self.scheduler
            block_size = int(getattr(scheduler, "block_size", 0) or 0)
            num_gpu_blocks = int(
                getattr(getattr(scheduler, "cache_config", None),
                        "num_gpu_blocks", 0) or 0)
            num_free_blocks = int(
                scheduler.kv_cache_manager.block_pool.get_num_free_blocks())
            usage = float(scheduler.kv_cache_manager.usage)
            ...
            return {
                ...
                "usage": usage,
                "total_tokens": usable_blocks * block_size,
                "free_tokens": num_free_blocks * block_size,
            }
    ...
    def preprocess_add_request(
        ...
        # MARS process request in from_engine_core_request, not in core.py
    ...
    def run_engine_core(*args,
        ...
        def signal_handler(signum, frame):
            ...
            if not shutdown_requested:
                ...
                logger.warning("EngineCore received signal %s; initiating "
                               "graceful shutdown.", signum)
                raise SystemExit()
